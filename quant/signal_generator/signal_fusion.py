...
class SignalFusion:
    ...
    def __init__(
        ...
        logger.info("signal_fusion_initialized ml_weight=ml_weight technical_weight=technical_weight llm_weight=llm_weight")
    ...
    def fuse_signals(
        ...
        if not any([technical_signal, ml_signal, llm_signal]):
            logger.warning("all_signals_missing symbol=symbol timestamp=timestamp")
        ...
        if not is_consistent:
            logger.info("signal_inconsistent directions=directions symbol=symbol timestamp=timestamp")
            return self._create_hold_signal(
                technical_signal, ml_signal, llm_signal,
        ...
        logger.info("signal_fused direction=final_direction strength=strength symbol=symbol timestamp=timestamp")
    ...
    def _normalize_signals(
        ...
        - ml: prediction > 0.0008 → buy, < -0.0008 → sell, ~0 → hold
        ...
        if technical and "signal" in technical:
            ...
            if tech_signal == 1:
                ...
                tech_dir = tech_signal

            if tech_dir not in ["buy", "sell", "hold"]:
                logger.warning(f"invalid_technical_signal signal={tech_signal}")
                tech_dir = None
    ...
    def _calculate_strength(
        ...
        if ml_signal and "prediction" in ml_signal:
            ...
            # 判断ML方向
            if prediction > 0.0008 and final_direction == "buy":
        ...
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    logger.info("fused_signal_saved symbol=fused_signal.get('symbol') direction=fused_signal.get('direction') strength=fused_signal.get('strength')")
                    return True

        except Exception as e:
            logger.error(f"fused_signal_save_failed error={e} signal={fused_signal}")
            return False
    ...
    def create_table_if_not_exists(self):
        ...
        try:
            ...
            logger.error("fused_signals_table_creation_failed error=str(e)")
            raise
