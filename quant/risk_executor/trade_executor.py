"""
交易执行器
将交易意图转换为订单，并通过 td_api 真实发单（支持 CTP 或天勤 TqKq）
"""
import logging
...
try:
    ...
    # 使用字符常量（用于天勤或测试）
    logger.warning("openctp-ctp 未安装，使用字符常量（天勤模式）")
...
class Order:
    ...
    def __init__(
        ...
        self.fill_result: Optional[Dict] = None
    ...
    def to_ctp_params(self) -> Dict:
        """
        转换为 CTPTradeApi/TqTradeApi.send_order() 所需的参数字典

        Returns:
            与 send_order() 签名匹配的关键字参数字典
        """
        return {
...
class TradeExecutor:
    ...
    def __init__(self, config, td_api=None):
        ...
        Args:
            ...
            td_api: CTPTradeApi 或 TqTradeApi 实例（可选）。
    ...
    def execute_order(self, intent: TradeIntent, price: Optional[float] = None) -> Optional[Order]:
        """
        执行订单：将 TradeIntent 转为 Order，并通过 td_api 发送。
        ...
        if self.td_api is not None:
            ...
            logger.info(f"发送报单，参数：{params}")
            try:
                order_ref = self.td_api.send_order(**params)
                order.order_ref = order_ref
                
                if order_ref:
                    logger.info(f"报单已提交，order_ref={order_ref}")
                    
                    # 天勤/CTP 通用：等待一下让订单状态稳定
                    import time
                    time.sleep(2)
                    
                    # 天勤 API 没有 wait_for_order，CTP API 有
                    if hasattr(self.td_api, 'wait_for_order'):
                        from datetime import datetime
                        ...
                        if status == 'filled':
                            ...
                            logger.warning(f"⚠️ 等待成交超时（Ref={order_ref}），下轮同步真实持仓确认")
                            return None
                    else:
                        # 天勤模式：简单等待后认为成功
                        logger.info(f"✅ 订单已发送（天勤模式，不等待成交回报）")
                        from datetime import datetime
                        self.last_order_time = datetime.now()
                else:
                    logger.error(f"报单失败，order_ref 为空")
                    return None

            except Exception as e:
                logger.error(f"报单失败：{e}", exc_info=True)
                raise
        else:
            logger.warning("td_api 未配置，仅生成订单对象（dry-run）")
            logger.info(f"Dry-run 参数：{order.to_ctp_params()}")
