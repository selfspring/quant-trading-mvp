# Multi-Agent Dev Team v3.0 — Flexible Multi-Agent Development Team

## English

> Flexible setup for 2–10 agent collaborative teams on OpenClaw, with Anthropic Harness-inspired Context Reset, Iteration Loop, Structured Evaluation, and Planner Expansion.

### What's New in v3.0

| Feature | v2.x | v3.0 |
|---------|------|------|
| Context management | Compaction only | **Context Reset + Structured Handoff** |
| Quality assurance | Linear (dev→QA) | **Generator→Evaluator iteration loop (GAN-inspired)** |
| Evaluation | Ad-hoc | **Structured criteria with dimensions, thresholds & scoring** |
| Planning | Manual spec | **Planner auto-expansion (1-4 sentences → full SPEC)** |
| Workflow steps | No reset markers | **`contextReset` + `handoffOutput` per step** |
| Iteration control | None | **`evaluator` + `iterateUntil` per step** |
...
### v3.0 Features: Context Reset & Iteration Loop

#### Context Reset + Structured Handoff

Long-running agent sessions suffer from **context anxiety** — agents start wrapping up prematurely as context fills. The solution is **context reset**: terminate the current agent, capture state in a structured handoff file, then start a fresh agent session.

- Workflow steps with `contextReset: true` require the agent to write a handoff file before completing
- Handoff format: `teamtask/tasks/{task-id}/handoff-{step}-{role}.md`
- Format specification: `references/handoff-format.md`
- Handoff includes: completed work, current status, remaining TODOs, pitfalls, decisions, next-step instructions

#### Generator-Evaluator Iteration Loop (GAN-inspired)

Agents are bad at self-evaluation — they reliably praise their own work. Separating **generator** (does the work) from **evaluator** (judges it) creates a feedback loop that drives quality up.

- Steps with `evaluator` + `iterateUntil` enter an automatic iteration loop
- Each iteration: spawn generator → spawn evaluator → score → if below threshold, inject feedback → next iteration (fresh context)
- Evaluator uses structured criteria from `config/evaluation-criteria.json`
- Anti-bias rules prevent LLM leniency
- Loop orchestration logic: `references/iteration-loop.md`
...
```json
{
  ...
  "role": "backend",
  "evaluator": "qa",
  "iterateUntil": { "minScore": 7, "maxIterations": 5 },
  "contextReset": true,
  "handoffOutput": "handoff-3-backend.md"
}
```

#### Structured Evaluation Criteria

Three presets in `config/evaluation-criteria.json`:
- **fullstack**: functionality, codeQuality, design, originality, robustness
- **backend-api**: functionality, codeQuality, performance, security, documentation
- **data-pipeline**: correctness, reliability, performance, observability, maintainability

Each dimension has weight, threshold (minimum passing score), description, and fail signals.
Evaluator prompt template with anti-bias rules and few-shot scoring examples: `references/evaluator-prompt.md`

#### Planner Auto-Expansion

PM agent can expand 1-4 sentence descriptions into full product specs (SPEC.md) with:
- Product overview, core features, user flows, data model, sprint breakdown, success criteria
- Key constraint: define **what** to build, NOT **how** to build it (avoids error cascading)
- Prompt template: `references/planner-prompt.md`
...
│   └── evaluation-criteria.json  # v3.0: Structured evaluation presets
├── references/            # v3.0: Orchestration & prompt references
│   ├── handoff-format.md        # Structured handoff file format
│   ├── iteration-loop.md        # Generator-Evaluator loop logic
│   ├── evaluator-prompt.md      # QA/Evaluator agent prompt template
│   └── planner-prompt.md        # PM/Planner agent prompt template
├── templates/
│   └── SOUL-template.md   # SOUL.md template (v3.0: with Context Reset Protocol)
└── wizard/
...
| 特性 | v2.x | v3.0 |
|------|------|------|
| 上下文管理 | 仅 Compaction | **Context Reset + 结构化 Handoff** |
| 质量保障 | 线性（开发→QA） | **Generator→Evaluator 迭代循环（GAN 灵感）** |
| 评估标准 | 临时制定 | **结构化评估维度 + 阈值 + 打分体系** |
| 需求展开 | 手写 Spec | **Planner 自动展开（1-4 句话 → 完整 SPEC）** |
| 流程步骤 | 无 reset 标记 | **每步可配置 `contextReset` + `handoffOutput`** |
| 迭代控制 | 无 | **每步可配置 `evaluator` + `iterateUntil`** |

#### Context Reset + 结构化 Handoff

长时间运行的 agent 会出现 **上下文焦虑**（提前收工）。解决方案：彻底终止当前 agent → 用结构化 handoff 传递状态 → 全新 agent session 接力。

- `contextReset: true` 的步骤，agent 必须写 handoff 文件后结束
- Handoff 格式：`references/handoff-format.md`
- 路径模板：`teamtask/tasks/{task-id}/handoff-{step}-{role}.md`

#### Generator-Evaluator 迭代循环

Agent 自评会系统性偏乐观。分离 generator（做事）和 evaluator（评判）+ 多轮迭代 = 质量飞跃。

- 配置 `evaluator` + `iterateUntil` 的步骤自动进入迭代循环
- 每轮：spawn generator → spawn evaluator → 打分 → 未达标则注入 feedback → 下一轮（全新上下文）
- 编排逻辑：`references/iteration-loop.md`

#### 结构化评估标准

三套预设（`config/evaluation-criteria.json`）：fullstack / backend-api / data-pipeline
每个维度有权重、阈值、描述、失败信号。含反偏倚规则和 few-shot 评分示例（`references/evaluator-prompt.md`）。

#### Planner 自动展开

PM 角色可将 1-4 句话描述展开为完整产品 Spec，只定义"做什么"不定义"怎么做"。
Prompt 模板：`references/planner-prompt.md`
...
│   └── evaluation-criteria.json  # v3.0: 结构化评估预设
├── references/            # v3.0: 编排与 Prompt 参考
│   ├── handoff-format.md        # Handoff 文件标准格式
│   ├── iteration-loop.md        # Generator-Evaluator 循环编排逻辑
│   ├── evaluator-prompt.md      # QA/Evaluator Prompt 模板
│   └── planner-prompt.md        # PM/Planner Prompt 模板
├── templates/
│   └── SOUL-template.md   # SOUL.md 模板（v3.0: 含 Context Reset Protocol）
└── wizard/
    └── setup.js           # 交互式配置向导
```
