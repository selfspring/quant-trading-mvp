...
def fetch_llm_signal():
    """从 news_analysis 表读取最近 1 小时内最高优先级的 LLM 信号

    筛选条件：importance >= medium 且 confidence >= 0.6
    多条时取 importance 最高 + confidence 最高的那条

    Returns:
        dict: {"direction": "bullish|bearish|neutral", "confidence": 0.0-1.0}
        None: 无有效信号
    """
    importance_rank = {'high': 3, 'medium': 2, 'low': 1, 'irrelevant': 0}
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("""
                    SELECT direction, confidence, importance
                    FROM news_analysis
                ...
                if not rows:
                    logger.info("最近 1 小时无有效 LLM 新闻信号")
                    return None

                # 取 importance 最高 + confidence 最高的那条
                best = max(rows, key=lambda r: (importance_rank.get(r[2], 0), r[1]))
                direction = best[0]
                confidence = float(best[1])
                importance = best[2]

                logger.info(f"LLM 信号: direction={direction}, confidence={confidence:.2f}, importance={importance}, 候选数={len(rows)}")
                return {"direction": direction, "confidence": confidence, "importance": importance}

    except Exception as e:
        logger.warning(f"读取 LLM 信号失败: {e}")
        return None
...
def main():
    ...
    try:
            ...
            try:
                with tq_trade_session(config) as trade_api:
                    ...
                    if ctp_long == 0 and ctp_short == 0 and len(state.get('open_positions', [])) > 0:
                        logger.warning("天勤无持仓但 state 有记录，清空 state")
                        state['open_positions'] = []
                        save_state(state)
            except Exception as e:
                logger.warning(f"启动时 天勤同步失败: {e}")
        ...
        try:
            ...
            _fe = FeatureEngineer()
            _kline_with_features = _fe.generate_features(kline_data.copy())
            regime_detector = MarketRegimeDetector()
            regime_info = regime_detector.detect(_kline_with_features)
            regime = regime_info.get('regime', 'ranging')
            regime_confidence = regime_info.get('confidence', 0.5)
            logger.info(f"市场状态：{regime}, ADX={regime_info.get('adx', 0):.1f}, 状态置信度={regime_confidence:.2f}")

            # 根据市场状态动态调整置信度阈值
            base_threshold = 0.40
            if regime == 'trending':
                confidence_threshold = base_threshold - 0.05  # 趋势市放宽阈值
            elif regime == 'volatile':
                confidence_threshold = base_threshold + 0.10  # 高波动收紧阈值
                logger.warning("高波动市场，提高开仓门槛")
            else:  # ranging
                confidence_threshold = base_threshold
        except Exception as e:
            logger.warning(f"市场状态识别失败，使用默认阈值：{e}")
            regime = 'ranging'
            confidence_threshold = 0.40

        # 5.6 信号融合（技术 + ML + LLM）
        logger.info("获取 LLM 新闻信号...")
        llm_signal = fetch_llm_signal()

        # 构造技术信号（供融合层使用统一的 buy/sell/hold 表示）
        raw_signal = ml_signal.get('signal', 0)
        if raw_signal == 1:
        ...
        technical_signal = {
            "signal": technical_direction,
            "strength": ml_signal.get('confidence', 0.5)
        }

        # 根据市场状态动态调整融合权重（PRD 要求）
        if regime == 'trending':
            fusion_weights = {'technical_weight': 0.4, 'ml_weight': 0.5, 'llm_weight': 0.1}
        elif regime == 'volatile':
            fusion_weights = {'technical_weight': 0.2, 'ml_weight': 0.3, 'llm_weight': 0.5}
        else:  # ranging
            fusion_weights = {'technical_weight': 0.5, 'ml_weight': 0.4, 'llm_weight': 0.1}

        # LLM 高重要性 + 高置信度立即触发信号（PRD 要求）
        llm_override = False
        if llm_signal and llm_signal.get('importance') == 'high' and llm_signal.get('confidence', 0) >= 0.75:
            logger.info(f"LLM 高优先级信号触发! direction={llm_signal['direction']}, confidence={llm_signal['confidence']:.2f}")
            llm_override = True

        try:
            signal_fusion = SignalFusion(**fusion_weights)
            ...
            # 保存融合信号到数据��
            signal_fusion.save_to_db(fused)

            fused_direction = fused.get('direction', 'hold')
            fused_strength = fused.get('strength', 0.0)
            logger.info(f"融合信号: direction={fused_direction}, strength={fused_strength:.2f}, regime={regime}, weights={fusion_weights}")

            # 用融合结果覆盖 ml_signal 的方向，保留其他字段供下游使用
            # 注意：signal_processor 期望整数 (1=buy, -1=sell, 0=hold)
            if fused_direction == 'buy':
                ml_signal['signal'] = 1
            elif fused_direction == 'sell':
                ml_signal['signal'] = -1
            else:
                ml_signal['signal'] = 0
            ml_signal['fused_strength'] = fused_strength

            # LLM 高优先级覆盖时，提升置信度确保不被过滤
            if llm_override and fused_direction != 'hold':
                ml_signal['confidence'] = max(ml_signal.get('confidence', 0), 0.80)
                logger.info(f"LLM 覆盖：置信度提升至 {ml_signal['confidence']:.2f}")

        except Exception as e:
            logger.warning(f"信号融合失败，退化为纯 ML 信号: {e}")

        # 5.7 置信度过滤（仅拦截开仓，不拦截平仓）
        confidence = ml_signal.get('confidence', 0)
        has_position = len(state.get('open_positions', [])) > 0
        if confidence < confidence_threshold and not has_position:
            logger.info(f"置信度 {confidence:.2f} < {confidence_threshold:.2f} 且无持仓，跳过本轮")
            return 0
