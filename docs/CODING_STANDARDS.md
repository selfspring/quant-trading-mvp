...
### 8. K 线 availability / lineage

- `30m` 是否可用、是否可由 `1m` 严格聚合，必须落在独立 availability / lineage 层
- 禁止把派生 `30m` 直接混成“原生 30m 且无来源标记”
- 第一轮严格规则只允许 `1m` 覆盖 `100%` 的同桶聚合
- 遇到跨交易段 / 日历未知时，优先保守标注，不做静默补齐
