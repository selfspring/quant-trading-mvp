...
│
├── data_collector/      # 数据采集
│   ├── ctp_market.py    # CTP 行情接口
│   ├── ctp_trade.py     # CTP 交易接口
...
| `run_single_cycle.py` | 单次策略执行（cron 触发） |
| `run_ctp_collector.py` | CTP 长驻采集进程（交易时段运行） |
...
| `strategy_state.json` | 策略状态持久化（连败计数、持仓信息、止损止盈） |
| `collector.pid` | CTP 采集进程 PID 文件（防重复启动） |
