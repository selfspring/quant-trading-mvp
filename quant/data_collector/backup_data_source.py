...
class BackupDataSource:
    ...
    def _initialize_data_source(self):
        ...
        try:
            ...
            if ts_token:
                ...
                logger.info("backup_data_source_initialized source='tushare'")
                return
        except Exception:
            logger.warning("tushare_init_failed error=str(e)")
        ...
        try:
            ...
            logger.info("backup_data_source_initialized source='akshare'")
            return
        except Exception:
            logger.warning("akshare_init_failed error=str(e)")

        # 3. 回退到 CSV 模式
        logger.info("backup_data_source_fallback source='csv'")
    ...
    def _get_tushare_klines(
        ...
        try:
            ...
            if df.empty:
                logger.warning("tushare_no_data symbol=ts_symbol")
                return None
            ...
            logger.info("tushare_klines_fetched symbol=symbol, count=len(df)")
            return df[['timestamp', 'open', 'high', 'low', 'close', 'volume']]

        except Exception:
            logger.error("tushare_fetch_failed error=str(e)")
            return None
    ...
    def _get_akshare_klines(
        ...
        try:
            ...
            if df.empty:
                logger.warning("akshare_no_data symbol=ak_symbol")
                return None
            ...
            logger.info("akshare_klines_fetched symbol=symbol, count=len(df)")
            return df[['timestamp', 'open', 'high', 'low', 'close', 'volume']]

        except Exception:
            logger.error("akshare_fetch_failed error=str(e)")
            return None
    ...
    def _get_csv_klines(
        ...
        if not os.path.exists(csv_path):
            logger.warning("csv_file_not_found path=csv_path")
            return None
        ...
        try:
            ...
            for col in required_cols:
                if col not in df.columns:
                    logger.error("csv_missing_column column=col, path=csv_path")
                    return None
            ...
            logger.info("csv_klines_loaded symbol=symbol, count=len(df), path=csv_path")
    ...
    def generate_mock_klines(
        ...
        logger.info("mock_klines_generated symbol=symbol, count=count, base_price=base_price")
        return df
    ...
    def fetch_and_store(
        ...
        if df is None:
            logger.warning("no_data_to_store symbol=symbol")
            return False
        ...
        try:
            ...
            logger.info("klines_stored_to_db symbol=symbol, count=len(df), period=period")
            return True
